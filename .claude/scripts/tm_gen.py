#!/usr/bin/env python3
"""Generate the TM overlay assembly from reference/tm.bin.
...
BASE = 0x8956
DATA = open(Path(__file__).parent.parent.parent / 'reference/tm.bin', 'rb').read()
END = BASE + len(DATA)

INLINE = {
    0x8119: ('text',),          # MSG_PRINT
    0x8115: ('text',),          # MSG_AT
    0xA436: ('text',),          # POPUP_TEXT (framed) -> POPUP_FRAME + MSG_PRINT
    0xA43C: ('text',),          # POPUP_TEXT_NF (no frame) -> WIN_VIEW + MSG_PRINT
    0xB703: ('text',),          # MLIB_BLOAD: inline zero-terminated path text
